...
# Read CSV file
df = pd.read_csv('visualize/experiment_summary.csv')
...
plt.title('Grouped Bar Chart of Experiment Metrics')
plt.xticks(x + 1.5 * bar_width, experiment_names, rotation=30, ha='right')
plt.legend()
plt.ylim(0.2, 0.8)  # 设置y轴起始值为0.4，上限为1.0
plt.tight_layout()

# Save image
output_path = os.path.join(output_dir, 'experiment_summary_grouped_bar.png')
